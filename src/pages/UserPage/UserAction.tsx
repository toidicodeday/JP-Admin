...
const UserAction = () => {
    ...
    const getUserData = useCallback(
        async () => {
            ...
            setIsLoading(true)
            if (res && userID) {
                const userName = new Set(res.documents.filter(item => item.userID === userID).map(item => item.name))
                console.log(...userName)
                setUserName([...userName])
                // Lay danh sach khoa hoc Id
                const listCourseID = [... new Set(res.documents.filter(item => item.userID === userID)?.map((item) => item?.courseID))]
                // Call API lay thong tin courses
                const getCourse = Promise.all(listCourseID?.map((id) => api.course.getOneCourse(id)))
                // Set course Table
                getCourse.then((item: any) => setCourseData(item)).catch((err) => console.log(err))

                const listLessonID = [...res.documents.filter(item => item.courseID === activeCourseID)?.map(item => item?.lessonID)]
                const getLesonID = Promise.all(listLessonID?.map((id) => api.lesson.getOneLesson(id)))
                getLesonID.then((item: any) => setLessonData(item)).catch((err) => console.log(err))
    ...
    return (
        <div className="px-8 pb-10">
            <div className="py-5 flex justify-end items-center">
                <Button icon={<FiArrowLeft />} onClick={handleBack} className='float-right flex items-center gap-1' type='primary'>Back</Button>
            </div>
